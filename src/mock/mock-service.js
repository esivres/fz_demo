...
export const makeServer = ({environment = "development"} = {}) => {
    createServer({
        ...
        routes() {
            ...
            this.get("api/:type/:id", (schema, request) => {
                let modelName = request.params.type ;
                return schema[modelName].find(request.params.id).attrs;
            });


            this.patch("api/:type/:id", (schema, request) => {
                let modelName = request.params.type ;
                let attrs = JSON.parse(request.requestBody);

                return schema[modelName].find(request.params.id).update(attrs);
            });
            ...
            this.delete("api/:type/:id", (schema, request) => {
                let modelName = request.params.type;
